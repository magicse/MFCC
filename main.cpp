...
int main(int argc, char** argv) {
	...
	// Assuming config.fileList is a std::string
    std::string fileListPath = directory + "/" + config.fileList; 
	fList=fopen(fileListPath.c_str(),"r");
	...
	while(fgets(fileNameBuf,maxBuffLength,fList)!=NULL){
		...
		std::string pcmFile1Path = directory + "/" + pcmFile1;
        f = fopen(pcmFile1Path.c_str(), "rb");
...
//				if (j == 0)ShowVector(fbank);
	...
	//	for (int i = 1; i <= VectorSize(dpostProc); i++)dpostProc[i] *= 10.0;
		...
		std::string pcmFile2Path = directory + "/" + pcmFile2;
		f_result = fopen(pcmFile2Path.c_str(), "w");
